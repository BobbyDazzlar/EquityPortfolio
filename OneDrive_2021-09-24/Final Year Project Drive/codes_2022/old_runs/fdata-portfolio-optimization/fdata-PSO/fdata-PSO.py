...
### 2>ASSIGN RADOM POSITIONS AND VELOCITiES TO EACH OF THE ASSET IN EACH OF THE PORTFOLIO
### 3>ASSUME THAT THE FIRST PORTFOLIOTO BE THE  BEST AND LET IT BE GBEST.
### 4>THEN UPDATE THE POSTIONS OF ALL THE ASSETS IN A PORTFOLIO AND COMPARE WITH THEIR PREVIOUS POSTIONS  WRT SHARPE RATIO.S
...
df = pd.read_csv("/home/pn_kumar/Karthik/fdata-portfolio-optimization/fdata-ACO/fdata08_02_22.csv")
...
fdata = pd.read_csv('/home/pn_kumar/Karthik/fdata-portfolio-optimization/fdata-ACO/fdata08_02_22.csv', parse_dates=['Date'], index_col='Date')
fdata = fdata.iloc[:row, :]

trading_days = len(fdata)
...
start_date = fdata.index[0]  # date of first row in the dataset (datetime format)
end_date = fdata.index[len(fdata) - 1]  # date of last row in the dataset  (datetime format)
...
daily_returns = []
for i in fdata:
    t = list(fdata[i])  # finding the close price col for each asset
    close.append(t)
    name.append(i)
    t = list(fdata[i].pct_change())  # daily return for stock
...
x = fdata.shape[0]  # FINDING NO OF ROWS, THE REASON IS EACH STOCK IS REPEATING X TIMES
...
for counter, symbol in enumerate(fdata.columns):
